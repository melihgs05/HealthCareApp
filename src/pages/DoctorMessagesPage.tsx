...
const DEMO_MESSAGES: MessageDTO[] = [
  {
    id: 'msg-001', fromId: 'demo-patient-001', toId: 'demo-doctor-001',
    from: 'Alex Johnson', subject: 'Medication question', body: 'Hello Dr. Carter, I wanted to ask if I can take Lisinopril with food. I have been experiencing some nausea.',
    date: '2026-03-01T09:15:00', read: false, parentId: null,
  },
  {
    id: 'msg-002', fromId: 'p-002', toId: 'demo-doctor-001',
    from: 'Maria Gomez', subject: 'Appointment request', body: 'Hi, I would like to schedule a follow-up for my blood pressure check.',
    date: '2026-02-28T14:30:00', read: true, parentId: null,
  },
  {
    id: 'msg-003', fromId: 'p-003', toId: 'demo-doctor-001',
    from: 'James Lee', subject: 'Lab results', body: 'I received the lab results notification but I can\'t understand them. Could you explain?',
    date: '2026-02-25T11:00:00', read: true, parentId: null,
  },
]
...
export function DoctorMessagesPage() {
  ...
  const cardCls = 'rounded-3xl bg-white shadow-sm ring-1 ring-emerald-100 dark:bg-slate-800 dark:ring-emerald-900/40'
          ...
          <div className="flex w-full flex-col border-r border-slate-100 dark:border-slate-700 md:w-72 lg:w-80">
            <div className="p-3 border-b border-slate-100 dark:border-slate-700">
              <input
                ...
                className="w-full rounded-xl border border-slate-200 bg-slate-50/60 px-3 py-1.5 text-xs outline-none focus:border-emerald-500 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-100"
              />
            </div>
                ...
                filtered.map((m) => (
                  <button
                    ...
                    className={[
                      ...
                      selected?.id === m.id ? 'bg-emerald-50 dark:bg-emerald-900/10' : '',
                    ].join(' ')}
                  >
                    <div className="flex items-center justify-between">
                      ...
                      {!m.read && <span className="ml-1 h-1.5 w-1.5 flex-shrink-0 rounded-full bg-emerald-500" />}
                    </div>
              ...
              <div className="flex flex-1 flex-col overflow-y-auto p-5">
                ...
                <form onSubmit={(e) => void handleReply(e)} className="mt-4 border-t border-slate-100 pt-3 dark:border-slate-700">
                  ...
                  <textarea
                    ...
                    className="mt-1 w-full resize-none rounded-xl border border-slate-200 bg-slate-50/60 px-3 py-2 text-xs outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-100"
                  />
